...
public class TourneeServiceImpl implements TourneeService {
    ...
    // Update the efficiency calculation to work with seconds
    private double calculerEfficacite(Tournee t) {
        ...
        // Formula: (kg) / (km * hours) * 10, max 100%
...
public TourneeResponse demarrer(String id) {
    ...
    // ❌ REMOVE: tournee.setDateDebut(new Date());
    // Keep the original planned dateDebut as is
...
public TourneeResponse terminer(String id, TerminerTourneeRequest req) {
    ...
    // ❌ REMOVE: Date now = new Date();
    tournee.setStatut(StatutTournee.TERMINEE);
    // ❌ REMOVE: tournee.setDateFin(now);
    // Keep the original planned dateFin as is
...
public TourneeResponse annuler(String id) {
    ...
    // ❌ REMOVE: tournee.setDateFin(new Date());
    // Keep the original dateFin as is
